...
export const getCommentsForPost = (comments = [], postId) => {
  ...
  if (postId) {
    ...
    for (let i = 0; i < comments.length; ++i) {
      ...
      // eslint-disable-next-line
      if (comment.postid == postId) result.push(comment);
    }
  }
  return result;
};
...
export const getUserNameForPost = (userId, users = []) => {
  ...
  if (userId) {
    ...
    for (let i = 0; i < users.length; ++i) {
      ...
      if (user.id == userId) result.push(user);
    }
  }
...
export const getUserPosts = (userid, posts = []) => {
  ...
  if (userid) {
    ...
    for (let i = 0; i < posts.length; ++i) {
      ...
      if (post.userid == userid) usersPosts.push(post);
    }
  }
  return usersPosts;
};
...
export const getUserComments = (userid, posts = [], comments = []) => {
  ...
  if (userid) {
    ...
    for (let i = 0; i < comments.length; ++i) {
      ...
      if (comment.userid == userid) userComments.push(comment);
    }
  }
  return userComments;
};
...
export const findUserConnectionInfo = (user, users = []) => {
  ...
  if (user) {
    ...
    for (let i = 0; i < users.length; ++i) {
      let user = users[i];
      if (user.id == user.followerid) result.push(user);
    }
  }
